...
- **Query Parameters**:
  ...
  - `status`: enum (optional) - Filter theo status: PENDING, REVIEWING, ACCEPTED, REJECTED, CANCELLED

### Xử lý
1. Lấy userId từ authenticated user
2. Parse query parameters (page, limit, status)
3. Query applications của user với relations (job, CV) và pagination
4. Return list applications với pagination info
