...
3. Kiểm tra job tồn tại
4. **Kiểm tra job status = PENDING** (chỉ approve job đang chờ duyệt)
5. Cập nhật job status = ACTIVE
...
- **Success Response** (200):
  ...
  ```
- **Error Responses**:
  - 400: Invalid job ID format / Only pending jobs can be approved
...
2. Kiểm tra user là ADMIN
3. Kiểm tra job tồn tại
4. **Kiểm tra job status = PENDING** (chỉ reject job đang chờ duyệt)
5. Cập nhật job status = SUSPENDED
6. Lưu reason nếu có (hiện tại chưa được lưu vào database)
7. Return updated job data
...
- **Success Response** (200):
  ...
  ```
- **Error Responses**:
  - 400: Invalid job ID format / Only pending jobs can be rejected
  - 403: Unauthorized / Admin access required
  - 404: Job not found
  - 500: Server error
...
## 17. Check Job Saved (Authenticated)

- **Method**: `GET`
- **URL**: `/api/jobs/:id/saved`
- **Authentication**: Required
...
### Xử lý
1. Validate job ID format
2. Kiểm tra user đã lưu job này chưa
...
## 18. Get Recommended CVs For Job (Recruiter/Admin)

- **Method**: `GET`
- **URL**: `/api/jobs/:jobId/recommended-cvs`
- **Authentication**: Required
- **Authorization**: RECRUITER or ADMIN role
...
### Xử lý
1. Validate job ID format
2. Kiểm tra user role (RECRUITER hoặc ADMIN)
3. Lấy danh sách CV phù hợp với job
4. Return list CVs
...
- **Success Response** (200):
  ...
  ```
- **Error Responses**:
  - 400: Invalid job ID format / Limit must be between 1 and 100
  - 401: Unauthorized
  - 501: Feature not available (nếu CV module chưa được migrate)
  - 500: Server error

**Note:** Feature này có thể trả về 501 Not Implemented nếu CV module chưa sẵn sàng.
