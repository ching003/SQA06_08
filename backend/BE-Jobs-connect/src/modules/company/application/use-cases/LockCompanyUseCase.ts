...
import type { LockCompanyInputDTO, LockCompanyOutputDTO } from '../dtos/index.js';
...
export class LockCompanyUseCase {
  ...
  async execute(input: LockCompanyInputDTO): Promise<LockCompanyOutputDTO> {
    // Check admin permission
    const admin = await this.userRepository.findById(input.adminId);
    if (!admin || admin.role !== UserRole.ADMIN) {
      throw new AuthorizationError('Only admins can lock companies');
    }
    ...
    // Check if company is already locked
    if (company.status === UserStatus.LOCKED) {
      throw new BusinessRuleError('Company is already locked');
    }
    ...
    // Lock all company jobs (bulk update)
    ...
    await this.notificationService.createNotification({
      userId: owner.userId,
      type: 'COMPANY_REJECTED', // Using existing notification type, can be extended later
      title: 'Công ty đã bị khóa',
      message: `Công ty "${company.name}" của bạn đã bị khóa bởi quản trị viên.`,
  ...
  private mapToOutput(company: any): LockCompanyOutputDTO {
    return {
      id: company.id,
      name: company.name,
